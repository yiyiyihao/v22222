...
jQuery(function() {
    ...
    uploader.on( 'uploadSuccess', function( file , response ) {
        ...
        //移除之前的图片
        $(uploaderId).parent().children('.thumbnail').remove();
        $(uploaderId).parent().parent().children('.thumbnail').remove();
        $(uploaderId).parent().append($li);
        ...
        if(response.code == '0'){
            ...
            $(uploaderId).parent().parent().children('input').val(response.url);
        }else{
            //失败
